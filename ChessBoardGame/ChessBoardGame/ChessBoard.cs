...
namespace ChessBoardGame
{
    public partial class ChessBoard : Form
    {
        ...
        private static bool botColor = true;
        private int turnCounter = 0;
        ChessPiece selectedPiece;
        private static int depthSearch = 5;
        ...
        void panelClick(object sender, EventArgs e)
        {
            ...
            if (potentialMoves != null)
            {
                if (potentialMoves.Contains(panelLocation))
                {
                    ...
                    if (botColor == this.whiteTurn)
                    {
                        redrawBoard();
                        this.Calculating.Visible = true;
                        this.Calculating.Refresh();
                        (int score, Point moveTo, ChessPiece bestChessPiece) = minimax.CalculateOptimalMove(chessPieces, botColor, depthSearch, int.MinValue, int.MaxValue, true);
                        this.Calculating.Visible = false;

                        foreach (ChessPiece tempChessPiece in chessPieces)
                        {
                            if (tempChessPiece.getLocation() == bestChessPiece.getLocation() &&
                                tempChessPiece.GetType() == bestChessPiece.GetType() &&
                                tempChessPiece.getColor() == bestChessPiece.getColor())
                            {
                                selectedPiece = tempChessPiece;
                                break;
                            }
                        }
